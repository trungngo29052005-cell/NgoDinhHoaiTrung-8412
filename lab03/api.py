...
def caesar_shift_char(c: str, key: int) -> str:
    if c.isalpha():
        base = 'A' if c.isupper() else 'a'
        return chr((ord(c) - ord(base) + key) % 26 + ord(base))
    return c

def caesar_encrypt_text(text: str, key: int) -> str:
    return ''.join(caesar_shift_char(c, key) for c in text)

def caesar_decrypt_text(text: str, key: int) -> str:
    return ''.join(caesar_shift_char(c, -key) for c in text)
...
def caesar_encrypt():
    ...
    key_raw = data.get('key', 0)
    try:
        key = int(key_raw)
    except Exception:
        return jsonify({'error': 'Invalid key'}), 400
    encrypted = caesar_encrypt_text(plain_text, key)
...
def caesar_decrypt():
    ...
    try:
        ...
        return jsonify({'error': 'Invalid key'}), 400
    decrypted = caesar_decrypt_text(cipher_text, key)
